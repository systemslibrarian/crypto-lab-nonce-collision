 ...
 * `#0b1512`, and with this lab's `--accent` (#b91c1c, a dark red) that resolves
 * to a near-black edge measuring 1.29:1 against
 ...
 * fix for its own accent-as-a-control failures introduced `--accent-strong` and
 * left `--accent` untouched precisely so these numbers did not move. Lifting
 * `--accent` itself to #d93030 would have changed the shared bar's ink and its
 * badge border in every repo that copies this block, which is not a change one
 * repo gets to make.
 */
...
> = {
  ...
  'control-boundary|a.cl-btn': { ratio: 1.29, required: 3, unverified: false },
