 ...
 *    an unknown backdrop, or a `color-mix()`. This lab is built out of them,
 *    and they are exactly the surfaces its verdicts are painted on: all three
 *    `.verdict-*` panels tint their surface (`color-mix(in oklab, var(--safe)
 *    12%, var(--surface))` and the `--warn` / `--broken` equivalents), the
 *    `.callout` does the same with `--accent`, `.bc-match` marks the identical
 *    CBC blocks with a `--warn` tint, `.cancel-term.shared` sits on a `--warn`
 *    tint over `--code-bg`, `.chip-toggle.is-active` on an `--accent` tint,
 *    `.reveal-lane .byte.printable` — the green that says "the crib fits here",
 *    the single most important colour in the lab — on a `--safe` tint, and the
 *    hero aside on `color-mix(in oklab, var(--accent) 6%, transparent)`. axe
 *    files every one of them under `incomplete`, so a violations-only gate
 *    measured the contrast of almost nothing this page uses to mean anything.
 ...
 *    at `.85` (a real fade, not one a later rule undoes) and `button:disabled`
 *    at `.6`.
 *
 ...
 *    boxes are the two `.byte-lane` regions (`max-height: 9rem; overflow: auto`,
 *    which every recovered byte lands in and overflows as soon as the messages
 *    are long enough), every `.table-scroll` and `.scoreboard-wrap`
 *    (`overflow: auto` around tables with a `min-width: 640px`, so they scroll
 *    at every viewport this gate uses). Content scrolled past those is not
 ...
 *    through to white, inventing a failure.
 *
 ...
 *    draws its one figure — the cancellation diagram — in HTML and CSS grid,
 *    and its only SVG is the shared header's two `aria-hidden` marks, so the
 *    sibling walk finds nothing to do. It is kept because the alternative is a
 *    helper that silently mismeasures the first labelled diagram anyone adds:
 *    an ancestor-only walk would composite a `<text>` onto the `<svg>`'s own
 *    transparent background rather than onto the `<rect>` drawn under it. The
 *    stroke-only guard matters for the same reason — the shared hamburger is
 *    three `<line>`s, and SVG's initial `fill` is black, so `getComputedStyle`
 *    reports an opaque black fill for geometry that paints none.
 *
 * TWO THINGS THIS WALK CANNOT SEE, stated so neither is mistaken for coverage.
 *
 * FIRST, generated content — and this page leans on it hard. Every non-colour
 * cue on a chip is a `::before`: `.chip.broken` is a `✗`, `.chip.safe` a `✓`,
 * `.chip.neutral` a `•`, `.inline-valid` a `✓`, `.inline-alarm` a `✗`,
 * `.chip-toggle` an `○` that becomes a `●` when active, and every
 * `details.card > summary` a `▸`/`▾` in `--accent-text` that REPLACES the UA
 * disclosure triangle (the same rule sets `list-style: none` and hides
 * `::-webkit-details-marker`). The walk iterates ELEMENTS and a pseudo-element
 * is not one, nor is it one to axe's `color-contrast` rule. `nontext.ts`
 * measures all of them, from the same paint model.
 *
 * SECOND, CSS-drawn shapes. The `.switch .slider::before` knob is
 * `content: ''` — a coloured circle with no glyph — so it is generated content
 * with no ink to attribute, and it is the ONLY visual difference between a
 * reused nonce and a fresh one on those four toggles. It is hand-measured in
 * `gate.ts`'s header instead.
 */
 ...
 * uses. This lab's only short-lived styling is the 0.5s `color` transition on
 * `.cancel-fadeable` and the matching opacity fade on `.cancel-gone` — and
 * under the reduced motion this gate asserts, the stylesheet collapses both to
 * 0.01ms, so the END state is on screen essentially immediately and there is no
 * half-faded frame to catch.
 *
 * The parameter is kept because the narrowed form is what makes a short-lived
 * state measurable at all: a class that lives half a second is shorter than a
 * full axe pass, so a whole-page scan would catch it only sometimes, and a
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * The expensive states here are the byte lanes: `renderByteLane` emits one
     * `<span class="byte">` per byte, so the combined lane and the reveal lane
     * are together a few hundred sibling elements all re-walking the same
     ...
     * that is both `.byte-lane` regions (`max-height: 9rem; overflow: auto`),
     * every `.table-scroll` and `.scoreboard-wrap` (`overflow: auto` around a
     * `min-width: 640px` table). `.sr-only` is handled separately below.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details class="card">` in the algebra section, both shut on arrival —
      // and each holds a cancellation diagram, a button and a computed-bytes
      // panel, so what they hide is a third of the page. The gate opens each by
      // clicking its <summary>, which is the route a reader has, rather than
      // setting `.open` from script — which is what the gate this replaces did
      // to every `<details>` on the page at once, before its single scan.
      ...
      // not merely inherited. `style.css` defines `.sr-only` with
      // `position:absolute; width:1px; height:1px; clip: rect(0,0,0,0)` — the
      // classic spelling, which `clippedToNothing` handles alongside the modern
      // `clip-path: inset(50%)`. Three things use it, and all are live during
      // the drive: `#cd-status`, an `aria-live="polite"` region whose text is
      // rewritten every time the two messages are re-encrypted, and the
      // `<caption class="sr-only">` on the consequence table and the
      // scoreboard. Each has a rect and passes `checkVisibility()` while
      // painting exactly zero pixels; measuring them reports ratios for ink
      // that was never laid down, and `#cd-status` would report a fresh one on
      // every scan. It is deliberately narrow: only a ZERO-AREA clip qualifies,
      // so a partially clipped element is still measured, worst case.
     ...
     *
     * What this lab hides is a closed set: `.v-icon` (the ✓/⚠/✗ opening a
     * verdict), `.sc-icon` (the same glyphs in the scoreboard cells), `.cc-icon`
     * (in a cancellation check), `.cancel-rule` (an empty 1px divider) and
     * `.slider` (the switch track). Every glyph among them is painted by the
     * SAME declaration as the words beside it — `.verdict-broken .v-title,
     * .verdict-broken .v-icon { color: var(--broken-text) }`, and `.sc-icon`
     * inherits from its `<td class="sc-broken">` — so measuring the words
     * measures the glyph too. That is literally true here rather than merely
     * likely, which is why it is written down.
     *
     * ONE EXCEPTION, and it is deliberate: `.cancel-gone` is `aria-hidden` and
     * carries the text `= 0`, the payoff of the cancellation diagram. It is
     * skipped by both oracles, so it was measured by hand — `--safe-text` on
     * `--code-bg`: 8.66:1 dark, 5.32:1 light. It is `aria-hidden` because the
     * `role="img"` label on the diagram above already states the cancellation
     * in words.
     *
     * Nothing else on this page hides a VALUE. No recovered byte, no key, no
     * tag and no verdict is inside an `aria-hidden` subtree — which was checked
     * rather than assumed, because that is the shared blind spot where both
     * oracles stop looking and it is the one place a live readout can hide from
     * a whole accessibility gate.
     */
     ...
     *
     * This lab has no SVG of its own — the cancellation diagram is HTML and CSS
     * grid. The guard stays because the usual way the mistake arrives is
     * exactly how this lab builds its DOM: `innerHTML` template literals in
     * `renderResult`, `renderScoreboard` and `blockCompareExtra`. Interpolating
     * a string ARRAY into one makes JS join it with commas, leaving a run of
     * "," text nodes inside whatever element wraps it — and inside a `<g>`
     * those render nothing while a walk that measured them would report a 1:1
     ...
     *
     * Today it is a no-op here, and that was checked rather than assumed:
     * `style.css` sets `html { background-color: var(--background) }` as a
     * LONGHAND — with a comment saying axe and WebKit miss the shorthand — so
     * the root's own paint is opaque and this branch returns it immediately.
     * That is exactly one declaration deep. The day it moves to a shorthand, or
     * a `height: 100%` appears on `html` or `body`, everything below the first
     * screen would report against a white page that does not exist — in a lab
     * whose dark palette bottoms out at `#0f1117`.
     */
