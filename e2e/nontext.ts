 ...
 *
 * WHAT THIS LAB PUT THROUGH IT. The 1.4.11 check this replaces lived in its own
 * spec file, `e2e/border.spec.ts`, and it queried exactly
 * `textarea, input[type='text']` — which is exactly the set the palette's
 * `--control-border` token was written for and correctly applied to. Pointing a
 * check only at the place a rule is already kept is the same as not having one:
 * every BUTTON, every `.chip-toggle` and every switch on this page took its edge
 * from `--border`, a SURFACE divider, and nothing ever measured it. It also
 * compared the border against the control's own FILL and never against the card
 * outside it, so a control could pass while dissolving into its panel — and it
 * took only `.first()`, so one passing control vouched for all of them.
 *
 * TWO CLASSES THIS FILE STILL CANNOT REACH, stated so neither is mistaken for
 * coverage, and hand-measured instead (numbers in `gate.ts`'s header):
 *  - CSS-drawn shapes with no glyph. `.switch .slider::before` is the knob of
 *    the four "reuse the nonce" toggles: `content: ''`, so it is generated
 *    content the pseudo-element pass skips for having no ink to attribute, and
 *    it is the only mark that distinguishes a reused nonce from a fresh one.
 *  - `input[type='range']`, painted by the UA with an author `accent-color`.
 *    The `appearance: auto` test below deliberately exempts native widgets, so
 *    the two sliders are measured by hand instead.
 */
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * The expensive states here are the byte lanes: one `<span class="byte">`
     * per byte, so the combined and reveal lanes are together a few hundred
     * sibling elements re-walking the same ancestors up to `<body>`. Without
     ...
     * what a page paints. Elsewhere in this fleet a lab whose dark canvas was
     * two radial gradients reported `rgba(0,0,0,0)` for both `html` and `body`;
     * the walk found nothing opaque, fell through to WHITE, and reported a
     * bright disclosure triangle at 3.95:1 against a page that is actually
     ...
     * theme. On THIS page every judged surface is a flat colour or a
     * `color-mix()`, both of which `background-color` reports — but the
     * gradient-aware sampler is what makes that a fact rather than an
     * assumption, and `.verdict-*`, `.callout`, `.bc-match` and
     * `.chip-toggle.is-active` are all mixes rather than literals.
     */
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // case the "is it trying to draw itself as a control?" test below already
