 ...
 *     x¹²⁸ + x⁷ + x² + x + 1
 *
 * Blocks are interpreted in GHASH's bit-reflected big-endian convention: byte
 * 0's most-significant bit is the coefficient processed first. This is a
 * from-scratch, dependency-free implementation so the AES-GCM nonce-reuse
 * "forbidden attack" genuinely computes on the same field GHASH uses. The unit
 * tests cross-check `ghashH` against @noble/ciphers' GHASH (an independent
 * reference implementation) so the field arithmetic is verified, not asserted.
 */
