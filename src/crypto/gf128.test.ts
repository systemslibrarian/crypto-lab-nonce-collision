...
import { ONE, bytesToField, fieldToBytes, gmul, gpow, ginv, gsqrt } from './gf128.ts';
...
describe('GHASH KAT: our field math matches @noble/ciphers reference', () => {
  it('ghashH equals noble ghash over empty AAD for random keys/lengths', () => {
    for (let i = 0; i < 40; i++) {
      ...
      input.set(ct, 0);
      new DataView(input.buffer).setBigUint64(input.length - 8, BigInt(ct.length) * 8n, false);
